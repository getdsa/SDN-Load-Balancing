...
class iplb(object):
    ...
    def _pick_server(self):
        ...
        #the weight of server Si (IP not sorted)
        #[server4, server1, server3, server2]
        serverWeight = [5,1,3,2]

        #selected server
        ipserver = self.total_connection.keys()[0]

        #define total connection
        totalconns = self.total_connection.values()
        #self.log.debug("jumlah koneksi:\n %s" % totalconns)

        #weighted least connection algorithm        
        for m in range (len(serverSet)):
            if (serverWeight[m] > 0):
                i = m+1
                for i in range (len(serverSet)):
