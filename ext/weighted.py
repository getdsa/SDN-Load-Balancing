...
class iplb(object):
    ...
    def _pick_server(self):
        ...
        #global serverWeight, totalconns, total
        '''if len(self.total_connection) == 0:
            return self.live_servers.keys()[0]
        ...
        '''for x in range(0, (serverCount)): 
            conn=totalconns.values()
            total.append(conn)
        self.log.debug("jumlah oneksi:\n %s" % total)'''
 
        serverSet = self.live_servers.keys()
        #the weight of server Si
        serverWeight = [1,2,3,5]

        #the current connection number of server
        ipserver = self.total_connection.keys()[0]
        totalconns = self.total_connection.values()
        #self.log.debug("jumlah oneksi:\n %s" % totalconns)
        ...
        for m in range (0, (serverCount)):
            if (serverWeight[m] > 0):
                ...
                for i in range (i, (serverCount)):
                    if (totalconns[m]*serverWeight[i] > totalconns[i]*serverWeight[m]):
                        m=i
                        #selected_server=serverSet[m]
                        ipserver = serverSet[m]
                        #totalconns = self.total_connection[m]
                return ipserver
        return None
