...
class iplb(object):
    ...
    ''''selected_server=0
    resourceUsedPercentage=[0,0,0,0]
    resourceServerPercentage=[1,2,4,5]
    connectionWeight = 0,01'''


    def _pick_server(self, key, inport):
        selected_server=0
        #Supposing there is a server set
        #serverName=[server1,server2,server3,server4]
        serverSet = self.live_servers.keys()
        #the weight of server Si
        serverWeight=[1,3,6,12]

        #the current connection number of server Si
        #the sum of current connection numbers
        ipserver = self.total_connection.keys()[0]
        totalconns = self.total_connection[ipserver]
        serverCount = len(serverSet)
        #for (m = 0; m < n; m++) {
        for m in range (0, (serverCount)):
            if (serverWeight[m] > 0):
                i = m+1
                for i in range (i, (serverCount)):
                    if (self.total_connection[m]*serverWeight[i]>self.total_connection[i]*serverWeight[m]):
                        m=i
                        selected_server=serverSet[m]
                        ipserver = m
                return ipserver
        return None
        ...
        """
        #global selected_server, resourceUsedPercentage, resourceServerPercentage, connectionWeight
        
        
        '''m=0
        for m in range (m, (serverCount)):
            if (resourceServerPercentage[m] > 0):
                i = m+1
                for i in range (i, (serverCount)):
                    if (resourceUsedPercentage[m]*resourceServerPercentage[i]>resourceUsedPercentage[i]*resourceServerPercentage[m]):
                        m=i
                        selected_server=serverSet[m]
                        resourceServerPercentage[m]-=connectionWeight
                        resourceUsedPercentage[m]+=connectionWeight
